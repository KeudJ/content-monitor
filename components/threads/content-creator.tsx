...
export default function ContentCreator({ accounts, initialContent = '', onPublished }: Props) {
  ...
  async function publish() {
    ...
    if (scheduleMode && scheduledAt) {
      await fetch('/api/threads/schedule', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ accountId, content: selected, scheduledAt }),
      })
    } else {
      await fetch('/api/threads/publish', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ accountId, content: selected }),
      })
    }
    setPublishing(false)
    onPublished?.()
  ...
  return (
    <div className="grid md:grid-cols-2 gap-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-1">
          <Label className="text-sm">발행 계정</Label>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`text-sm px-3 py-1 rounded border transition-colors ${!scheduleMode ? 'bg-foreground text-background' : ''}`}
          >즉시 발행</button>
          <button
        ...
        <Button
          className="w-full"
          onClick={publish}
          disabled={publishing || !selected || !accountId}
        >
          {publishing ? <Loader2 className="h-4 w-4 mr-1.5 animate-spin" /> : null}
          {scheduleMode ? '예약 저장' : '발행하기'}
        </Button>
      </div>
    </div>
  )
